//钱包
const UUID = require('uuid'); //uuid 可以帮我们生成唯一的id
// 类的定义
// java  public private 
class Wallet {
    constructor () {
        // replace 替换功能 正则表达式替换查找  把- 换成""" 
        // js 对private支持不太好   使用编程的约定来做
        // _id 表示这个属性是不可以改的
    ...
    getBlance () {
        return this._balance; // 便于存储 long int 长整型
    }
    getCreateTime() { // 不可以
...
// 1. 每个人的钱包， 要有一个不可变的id  一串数字
// 钱 数字，属于你？  你在钱里钱包就是唯一id

const jayWallet = new Wallet();
// 使用
// jayWallet._id='aaaa'; //随意的修改？
// id 生成后 ？ 不能改啊   private 
// js 语言对面向对象支持不是那么大语言化， 原型式
// oo， 设计模式 Java c++
...
// 至少能去蘑菇街上班了
jayWallet.decreaseBalance(8);
console.log(jayWallet.getBlance());
jayWallet._id = 'aaa';
console.log(jayWallet.getId());
// console.log(jayWallet.getBlance(), jayWallet.getBalanceLastModifiedTime)
